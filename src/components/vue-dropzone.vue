...
<script>
    ...
    export default {
        ...
        watch: {
            existingFiles: function (files) {
                if (!Array.isArray(files) || files.length === 0) {
                    return;
                }

                this.setExistingFiles(files);
            }
        },
        computed: {
            dropzoneSettings() {
        ...
        mounted() {
            ...
            this.dropzone.on("totaluploadprogress", function (totaluploadprogress,
                                                              totalBytes,
                                                              totalBytesSent) {
                vm.$emit(
            ...
            this.setExistingFiles(this.existingFiles);
        ...
        methods: {
            ...
            setExistingFiles(files) {
                ...
                files.forEach((file) => {
                    ...
                    this.dropzone.files.push(file);
                    this.$emit("vdropzone-existing-file-complete", file);
                });
            }
        }
    };
